...
export default function Home() {
    const { userWorkouts, handleWorkoutCompletion, handleWorkoutIncompletion } = useContext(UserContext);
    ...
    useEffect(() => {
        ...
        setTempIsCompleted(todayWorkouts.length > 0 ? todayWorkouts[0].isCompleted : "");
    }, [userWorkouts, today]);
            ...
            <View style={Styles.sub_container_b}>
                <ScrollView contentContainerStyle={[Styles.scrollContainer, { marginTop: 20 }]}>
                    <BasicCardView
                        ...
                        subText_a={`Today: ${todayFocus.length === 0 ? "Rest Day" : todayFocus[0].name}`}
                        subText_b={`${todayFocus.length === 0 ? "Rest is the sweet sauce of labor" : "Nothing worth having comes easy."}`}
